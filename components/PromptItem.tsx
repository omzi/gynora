...
const PromptItem: FC<PromptItemProps> = ({ onClick, standalone, nonStandalone, disabled }) => {
	return (
		<button
			disabled={disabled}
			onClick={() => onClick(`${standalone} ${nonStandalone}`)}
			className='relative bg-transparent hover:bg-black/10 dark:hover:bg-white/10 focus:bg-black/10 dark:focus:bg-white/10 disabled:bg-muted disabled:cursor-not-allowed border group w-full whitespace-nowrap rounded-xl px-4 py-2 text-left text-black dark:text-white md:whitespace-normal'
		>
